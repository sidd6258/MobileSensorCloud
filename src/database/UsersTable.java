...
public class UsersTable {
	public List<UsersTableVO> display(){
		...
		try{
				...
				ResultSet rs = st.executeQuery("SELECT u.user_id,count(s.sensor_id) as NUMBER_SENSORS,u.user_email,u.user_fname,u.user_lname,u.user_status FROM sensorcloud.user_master u join sensorcloud.sensor_master s on (s.user_id=u.user_id) group by user_id;");
				while(rs.next()){
